...
class RateRadarBackground {
    ...
    async getCurrentRate(fromCurrency, toCurrency) {
        // Use cached rates instead of API calls - completely offline
        const cachedRates = {
            'USD/EUR': 0.85, 'EUR/USD': 1.18,
            'USD/GBP': 0.73, 'GBP/USD': 1.37,
            'USD/NGN': 410.0, 'NGN/USD': 0.0024,
            'USD/ZAR': 15.2, 'ZAR/USD': 0.066,
            'EUR/GBP': 0.86, 'GBP/EUR': 1.16,
            'EUR/NGN': 482.0, 'NGN/EUR': 0.0021,
            'EUR/ZAR': 17.9, 'ZAR/EUR': 0.056,
            'GBP/NGN': 561.0, 'NGN/GBP': 0.0018,
            'GBP/ZAR': 20.8, 'ZAR/GBP': 0.048,
            'NGN/ZAR': 0.037, 'ZAR/NGN': 27.0,
            'USD/JPY': 110.0, 'JPY/USD': 0.0091,
            'USD/CAD': 1.25, 'CAD/USD': 0.80,
            'USD/AUD': 1.35, 'AUD/USD': 0.74,
            'USD/CHF': 0.92, 'CHF/USD': 1.09,
            'USD/CNY': 6.45, 'CNY/USD': 0.155
        };
        
        const rateKey = `${fromCurrency}/${toCurrency}`;
        const rate = cachedRates[rateKey];
        
        if (rate) {
            return rate;
        } else {
            throw new Error('Currency pair not supported offline');
        }
    }
